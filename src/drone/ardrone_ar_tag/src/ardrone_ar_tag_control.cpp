...
ArdroneARTag::ArdroneARTag(std::string navdata_topic, std::string cmd_topic, std::string ar_tag_front_topic, std::string ar_tag_bottom_topic, double hz)
{
    ...
    double* k_p = new double[4] {2, 0.5, 1, 3};
    double* k_d = new double[4] {0, 0.3, 0.2, 1.5};
    double* k_i = new double[4] {0.5, 0.0, 0.1, 0.0};
    double* crit = new double[4] {3, 3, 2, 1};
    double* max_int_rel = new double[6] {0.2, 0.0, 0.1, 0.0};
    controller_chasing = new ArdronePID(hz, k_p, k_d, k_i, crit, max_int_rel);
    
    k_p = new double[4] {1, 1, 1, 5};
    k_d = new double[4] {0.3, 0.3, 0.2, 2};
...
void ArdroneARTag::correct_necessary_pose_shift(void)
{
    ...
    //necessary_pose_shift.y = (is_spotted_bottom) ? dx_dy_rel.second : 0;
...
void ArdroneARTag::control(void)
{
    ...
    {
        ROS_INFO("%f, %f, %f", necessary_pose_shift.x, necessary_pose_shift.rot_z, (ros::Time::now() - last_spotted_time).toNSec() / 1000000000.0);
        ...
        if (pose_handler->get_state()!= 8 && pose_handler->get_state()!= 2)
